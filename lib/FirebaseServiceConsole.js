...
async function updateUser(uid, change, value) {
  ...
  await admin.auth().updateUser(uid, userData).then(function (userRecord) {
    // See the UserRecord reference doc for the contents of userRecord.
...
function removeAllUsers(nextPageToken) {
  admin.auth().listUsers(100, nextPageToken).then(function (listUsersResult) {
    ...
    if (listUsersResult.pageToken) {
      removeAllUsers(listUsersResult.pageToken);
    }
  }).catch(function (error) {
  ...
  fs.readFile(filename, (err, data) => {
    ...
    if (records.length > 0) {
      for (let row = 0; row < records.length; row++) {
        const displayName = records[row][0].replace(/\s/g, '');
        const email = records[row][1].replace(/\s/g, '');
        const pwd = records[row][2].replace(/\s/g, '');
        createUser(email, pwd, displayName);
      }
    }
  });
}
